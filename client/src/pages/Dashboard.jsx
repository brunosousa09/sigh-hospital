...
import { 
  ...
  RefreshCw, TrendingUp, TrendingDown, Code, Briefcase, Eye, User
} from 'lucide-react';
          ...
          <div className={`p-5 sm:p-6 rounded-2xl border bg-slate-900 shadow-xl transition-all hover:-translate-y-1 relative overflow-hidden group ${kpiData.balance > 0 ? 'border-red-500/30 shadow-red-900/10' : 'border-green-500/30 shadow-green-900/10'}`}>
            ...
            <div className="relative z-10">
              <div className="flex justify-between items-start mb-4">
                <div><p className="text-slate-400 text-xs sm:text-sm font-medium mb-1">Saldo Devedor</p><h3 className={`text-2xl sm:text-3xl font-bold font-exo ${kpiData.balance > 0 ? 'text-red-400' : 'text-green-400'}`}>{formatMoney(kpiData.balance)}</h3></div>
              ...
              <div className="flex items-center gap-2 text-xs text-slate-500">{kpiData.balance > 0 ? <><TrendingUp size={14} className="text-red-400" /> Dívida Ativa</> : <><TrendingDown size={14} className="text-green-400" /> Contas em dia</>}</div>
            </div>
          </div>
